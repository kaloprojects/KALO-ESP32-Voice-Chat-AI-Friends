...
// ------------------------------------------------------------------------------------------------------------------------------
// ----------------             KALO Library - OpenAI_Croq_LLM AI Chat-Completions call with ESP32               ----------------
// ----------------                               Latest Update: Aug. 11, 2025                                   ----------------
// ----------------                                      Coded by KALO                                           ----------------
// ----------------                                                                                              ----------------
// ----------------                               # [NEW] since August 2025 #                                    ----------------
// ----------------          # Multiple AI characters (FRIENDS) supported, calling by friends 'name' #           ----------------
// ----------------          # More than 20 LLM models supported, OpenAI and [NEW]: CroqCloud server #           ----------------
// ----------------          # LLM Croq Meta [llama-3.1-8b-instant] 2-3x faster than earlier Open AI #           ----------------
// ----------------                                                                                              ----------------
...
// ----------------  CALL: String Result = OpenAI_Croq_LLM(UserRequest, LLM_OA_KEY, flg_WebSearch, LLM_CQ_KEY)   ----------------
...
String  MESSAGES;                // MESSAGES contains complete Chat dialog, initialized with SYSTEM PROMPT from active FRIENDS[]
                                 // .. each 'OpenAI_Croq_LLM()' call APPENDS new content (and erased on each 'friend' change)
                                 // {"role": "user", "content": ".."},
...
// ------------------------------------------------------------------------------------------------------------------------------
// [NEW]: Versatile LLM call, supporting multiple LLM models via OPEN AI or [NEW]: CROQ CLOUD server API calls
// Supporting dozens of LLM models (OpenAI: https://platform.openai.com/docs/models | Crog: https://console.groq.com/docs/models)
// Default setting below for CHAT LLM:   Croq Meta [llama-3.1-8b-instant]       <- very FAST, low latency, lowest costs
...
// 
// - OpenAI_Croq_LLM() 'remembers' all conversations during session on ongoing dialogs (appending last I/O to String MESSAGES)
...
//            - flg_WebSearch:  false -> using CroqCloud CHAT LLM (Croq llama-3.1-8b-instant)
//            - flg_WebSearch:  true  -> using OpenAI WEB SEARCH  (OpenAI gpt-4o-mini-search-preview) & WEB_SEARCH_USER_CITY
//            - llm_groq_key:   User registration and Croq API key needed, check Groq website for more details 
// RETURN:    LLM String response
//
// Examples:  String answer = OpenAI_Croq_LLM( "What means RGB?", "OA_KEY", false, "CROQ_KEY" );        <- default CROQ chat
//            String answer = OpenAI_Croq_LLM( "Will it rain tomorrow?", "OA_KEY", true, "CROQ_KEY" );  <- OpenAI websearch chat
//
// = Links =
...
//
// CroqCloud: - API CHAT COMPLETION: https://console.groq.com/docs/api-reference#chat
...
{   
    ...
    UserRequest.replace("Hashtag.", "#");                             // optionally (for STT): convert spoken 'Hashtag.' to '#'
    if (UserRequest == "#")                                           // if DONE: Leave. (no LLM launched)
    ...
    /* Some take aways & experiences from own tests / testing other models with OpenAI or Croq server
    ...
    if (llm_groq_key == "" || flg_WebSearch)                          // using #OPEN AI# only for web search (or if no CROQ Key)
...
// - All Agent related tasks as collected here to keep the main 'OpenAI_Croq_LLM()' smart & clean. 
// - CALL: only in 'OpenAI_Croq_LLM()', function is not needed (and could be removed) in case 1 Agent (FRIEND[0]) used only
...
void WakeUpFriends( String UserRequest, bool flg_WebSearch )
{
    ...
    {  gl_CURR_FRIEND_before = gl_CURR_FRIEND;  
       ...
       MESSAGES += FRIENDS[gl_CURR_FRIEND].prompt;            // in Open AI & Croq syntax: {"role": "system", "..prompt.."}  
       MESSAGES += "\"}";                        
    }

    // ------ Check command [#]: List all available FRIENDS (and current active friend) in Serial.Monitor, also flash ## LED
   
    UserRequest.replace("Hashtag.", "#");                     // optionally (for STT): converts spoken 'Hashtag.' to '#'
    if ( UserRequest == "#" )          
    {  led_RGB(LOW,HIGH,HIGH); delay(200);                    // LED: ## RED Flash (200ms) to indicate KEYWORD '#'    
    ...
    // ------ finally: restore correct RGB LED status always (after flashing ## RED above)
...
// CALL:     Call function on demand (used once in OpenAI_Croq_LLM() for waking up (initializing) any friend (calling by name)
