...
#define  VERSION               "\n== KALO ESP32 Voice ChatGPT (last update: June 5, 2025) ==================================\n"   
...
// --- NEW: hardware PCB specific toggles  --- 

#define TECHIESMS_PCB     true    // set to 'true' if techiesms 'Portable AI Assistant' pcb used 
                                  // Supporting techiesms pcb: https://techiesms.com/product/portable-ai-voice-assistant/
                                  // (false on KALO default pcb with audio volume poti)
...
int gl_TOUCH_RELEASED;            // measured once in setup(), typical values: untouched/empty ~50, when touched about 10-30 
                                  // Rule (by own tests): triggering if current value is below 80% of gl_TOUCH_RELEASED
...
// Be AWARE: functions below are RE-NAMED and have additional PARAMETERS (since latest code update): 
...
{   
  ...
  {  if ( SD.exists( WELCOME_FILE ) )
     {  audio_play.setVolume(8);    // supported values: 0-21. We play file welcome file with fixed reduced volume (e.g. 8)   
...
void loop() 
{
  ...
  // #NEW: Update: Utilizing new 'real-time' web search feature in case user request contains the keyyword 'GOOGLE' 
...
{   
   ...
   /* in case you prefer free GOOGLE TTS service (free of usage, but less 'human' and limited in sentence length !)
   ...
   //   ==> So CHECK your library & update this last line in code !! ....
