...
// ----------------        [Major changes: language & API key parameter added, KeepAlive() removed]              ----------------   
// ----------------                                                                                              ---------------- 
...
// ----------------                                                                                              ----------------   
// ----------------     CALL: transcript = SpeechToText_Deepgram( SD_filename, user_language, DEEPGRAM_KEY)      ----------------
// ----------------     [no Initialization needed, user language is optional (keep empty for mulitlingual)]      ----------------
// ----------------                                                                                              ----------------
// ----------------                           Prerequisites: DEEPGRAM API KEY needed                             ----------------
// ------------------------------------------------------------------------------------------------------------------------------
...
// *** HINTS to Deepgram usage: 
// - Deepgram supports multi lingual audios (automated voice language detection) and dedicated (user defined) languages 
// - SpeechToText services are AI based, means 'whole sentences with context' typically have a much higher recognition
//   quality (accuracy) than a sending single words only ! (e.g. a single 'Hi' audio won't work well in multi lingual mode)
// - Forcing to a dedicated user language improves speed an quality of recognition too (see 'user_language' param below). 
// - Optional STT_KEYWORDS param below allows to add dedicated words on top of the vocabulary
...
/* #include <SD.h>              // library needed, but already included in main.ino: */


// --- user preferences --------  

#define TIMEOUT_DEEPGRAM   10   // max. waiting time [sec] for Deepgram transcription response, e.g. 10 seconds     

#define STT_KEYWORDS            "&keywords=KALO&keywords=TECHIESMS"   // optional: adding any user words to vocabulary


// --- defines & macros --------

#ifndef DEBUG                   // user can define favorite behaviour ('true' displays addition info)
#  define DEBUG false           // <- define your preference here [true activates printing Deepgram detail]  
...
String SpeechToText_Deepgram( String audio_filename, String user_language, const char* deepgramApiKey )
{ 
  // Comment on 'user_language': Deepgram supports multilingual audio (supporting multiple languages, auto detection)
  // - using empty String (""):  Deepgram detects and transcribes multiple languages automatically
  // - user defined language (e.g. "en-US"): Deepgram 'understands' one language only .. benefit: faster & more accurate
  //   abbreviations e.g. "en", "en-US", "en-IN", "de" .. https://developers.deepgram.com/docs/models-languages-overview 
  ...
  { DebugPrintln("> Initialize Deepgram Server connection ... ");
    ...
    { Serial.println("\nERROR - WifiClientSecure connection to Deepgram Server failed!");
      ...
      return ("");   // in rare cases: WiFiClientSecure freezed rarely (with older libraries) 
    }
  ...
  // ### meanwhile no longer needed as we don't use 'Deepgram_KeepAlive() any longer, just keeping as reference
  ...
  optional_param += (user_language != "") ? ("&language=" + user_language) : ("&detect_language=true");    
  ...
  { while (client.available())                         
    ...
    // printing dots '.' each 500ms while waiting (2025-01 Update: each 500ms, even if DEBUG false)
    /* earlier July 2024 version: DebugPrint(".");  delay(100); */ 
    Serial.print(".");                   
    delay(250);                                 
  } 
  ...
  client.stop();     // observation: unfortunately needed, otherwise the 'audio_play.openai_speech() in AUDIO.H not working !
                     // so we close for now, but will be opened again on next call (and optionally via Deepgram_KeepAlive())
...
/* // ---------------------------------------------------------------------------------------------------------------------------
// ## meanwhile KeepAlive() no longer needed (WiFiClientSecure is stable, Deepgram speed improved), just kept as history note  ## 

void Deepgram_KeepAlive()    // idea was to be called e.g. each 5 seconds (to overcome default autoclosing after 10 secs)
{
  uint32_t t_start = millis();  
