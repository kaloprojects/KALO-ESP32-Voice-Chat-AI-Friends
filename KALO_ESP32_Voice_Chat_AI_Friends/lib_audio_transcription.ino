...
// - [##NEW with Sept. 2026 Update]: using model 'scribe_v2' (instead 'scribe_v1'), server endpoint still: "api.elevenlabs.io"
//
...
String SpeechToText_ElevenLabs( String audio_filename, uint8_t* PSRAM, long PSRAM_length, String language, const char* API_Key )
{
  ...
  { DebugPrintln("> Initialize ELEVENLABS Server connection ... ");
    ...
    { Serial.println("\n* ERROR - WifiClientSecure connection to Server failed!");
      ...
      return ("");   // in rare cases: WiFiClientSecure freezes (with older libraries)
    }
    DebugPrintln("Done. Connected to ELEVENLABS Server.");
  }
  client.setNoDelay(true);            // immediately flush after each write(), means: disable TCP nagle buffering.
                                      // Idea: might increase performance a bit (20-50ms per write) [default is false]

  uint32_t t_connected = millis();

  // ---------- Check if AUDIO file exists, check file size (PSRAM supported)
  ...
  // [model] (mandatory):            [##NEW with Sept. 2026 Update] using model 'scribe_v2' (instead 'scribe_v1')
  ...
  client.println("POST /v1/speech-to-text HTTP/1.1");     // still correct (also with  model 'scribe_v2')
  ...
  client.stop();    // alternative: if ( ESP.getPsramSize() == 0 ) {client.stop();}
  ...
  DebugPrintln( "ELEVENLABS: [scribe_v2] - " + ((language == "")? ("MULTI lingual") : ("MONO lingual [" + language +"]")) )
