...
The device works per default in _multi lingual_, means each chatbot/FRIEND can automatically _understand and speak in multiple languages_, no changes in code needed or system prompts needed. Also mixed usage is supported (changing language in same dialog session). List of supported languages (Aug. 2025): 99 languages in [STT](https://elevenlabs.io/docs/capabilities/speech-to-text#supported-languages), 57 languages for [TTS](https://platform.openai.com/docs/guides/speech-to-text/supported-languages#supported-languages).
...
# Github Updates
- **2025-08-11:** Major update (see above). Supporting multiple **custom chatbots/FRIENDS**, LLM AI response **2x faster via GroqCloud API**
- **2025-06-28:** Hardware **pin assignments** cleaned up (3 PCB examples in main.ino), Techiesms [Portable AI Voice Assistant](https://techiesms.com/product/portable-ai-voice-assistant/) supported by default (no longer #define TECHIESMS_PCB true needed). Added audio volume **VOL_BTN** button to all devices without analogue POTI
...
- **2025-06-05:** Major update, detail see above (**PSRAM** support, **ElevenLabs Scribe v1 STT** added support, STT **performance** increased)
...
- RGB color (recap):  STT (cyan) -> LLM AI (blue) -> TTS (yellow>pink, looks white in video).  WHITE flash = Success. _NEW_: Double RED flash = calling _FRIEND_
