...
# Known issues
- Script is tested with ESP32-WROOM-32 (no PSRAM) and ESP32-WROVER (with 4MB PSRAM), both working well. The ESP32-S3 currently produces a Compiler Error ('_.. has no non-static data member named 'msb_right_'). I am assuming a slightly different i2s_std_slot_config_t struct definition on ESP32-S3). 2025-06-12 Update: Issue found, resolved in my latest (internal) code, will be published soon.
