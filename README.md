# Summary
Prototype of a ESP32 based voice chat dialog device, _similar Open AI ChatGPT via Voice_. Just as an use case of my earlier published [KALO-ESP32-Voice-Assistant Libraries](https://github.com/kaloprojects/KALO-ESP32-Voice-Assistant). User can record questions with a microphone _(pressing a button as long speaking)_, Open AI bot will answer via voice. Workflow: After releasing Recording button ESP32 is sending recorded audio to _STT (SpeechToText) Deepgram server_, then sending transcription to _Open AI server_ (LLM model e.g. "gpt-4o-mini"), finally converting Open AI answer with _TTS (TextToSpeech) server_ into AUDIO and playing response via speaker on ESP32.
