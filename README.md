...
# Known issues
- Script is tested with ESP32-WROOM-32 (no PSRAM) and ESP32-WROVER (with 4MB PSRAM), both working well. The ESP32-S3 currently produces a Compiler Error ('_.. has no non-static data member named 'msb_right_'). I will follow up on this next (assuming a slightly different i2s_std_slot_config_t struct definition on ESP32-S3)
- Voice instruction not working and 'isRunning()' issue still exist (LED still indicated Playing 1-2 secs after audio finished): Both issues _are_ fixed already, but ESP32 with PSRAM needed (due AUDIO.H dependencies, see below)
